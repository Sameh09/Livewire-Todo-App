<div wire:key="{{ $todo->id }}"
    ...
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex">
            ...
            @if ($todo_new_name === $todo->name)
            <div>
                <input wire:model="todo_new_name" type="text" placeholder="Todo.." class="bg-gray-100 text-gray-900 text-sm rounded block w-full p-2.5">
                @error('todo_new_name')
                    <span class="text-red-500 text-xs ml-2 mt-3 block">{{$message}}</span>
                @enderror
            </div>
            @else
    ...
    <div class="mt-3 text-xs text-gray-700">
        ...
        <button wire:click="cancel_edit" class="mt-3 px-4 py-2 bg-red-500 text-white font-semibold rounded hover:bg-red-600">Cancel</button> 
    </div>
    @endif
</div>
